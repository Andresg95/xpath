const {getXpathsforValue} = require('./src/dataStructure');
...
const main = async () => {
    ...
    let myalias = results.map((res, index)=>{return {
        ...
        Class: res.Class,
        clasValues: _.get(html, res.Class),
...
const recurrentExecution = async (source, results, html) => {
    ...
    const intervalObject = setInterval(async () => {
        ...
        if (count % 5 == 0) {
            ...
            let className = _.get(html, results[1].classValues)
            
            console.log({idValue});
            //let idValue = _.get(_html, _results[1].id)
            let spxpath = results.filter((o)=>o.alias==alias || o.idValue==idValue || o.classValues==className)[1];
            ...
            if(foundByxpath){
                ...
                let _newResults = _results.map((res)=>{return {
                    alias: "Myvalue",
                    value: res.value,
                    xpath: res.xpath,
                    id: res.id,
